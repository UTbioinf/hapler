...
public class Sequence {
	...
	private ArrayList<SNP> snpsCovered;
	...
	public String toString() {
		...
		for(int i = 0; i < snpsCovered.size(); i++) {
			SNP snpi = snpsCovered.get(i);
			if(i != snpsCovered.size()-1) {
	...
	public boolean sharesSNPWith(Sequence other) {
		...
		for(int i = 0; i < snpsCovered.size(); i++) {
			SNP snp = snpsCovered.get(i);
			//if(!snp.isMasked() && other.containsSNP(snp)) {
				return true;
			//}
		}
	...
	public ArrayList<SNP> getSNPsCovered() {
		return snpsCovered;
	}
	...
	public boolean shouldMask(Sequence other) throws Exception {
		...
		boolean toRet = true;
		ArrayList<SNP> otherSNPs = other.getSNPsCovered();

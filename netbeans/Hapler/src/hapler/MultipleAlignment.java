...
public class MultipleAlignment {
	...
	public String SNPsAsString() {
		...
		if(snps.size() != 0) {
			...
			sb.append(snps.get(snps.size()-1).getPosition());
		}
		return sb.toString();
	}
	...
	/**
	 * Does nothing if this sequence already exists
	...
	public void maskUnmaskedRedundantSeqsByUnmasked() throws Exception {
		...
		ArrayList<Sequence> toMask = new ArrayList<Sequence>();
	 ...
	 * @return
	 */
	public ArrayList<Sequence> maskUnmaskedNoSNPSequences(){
		ArrayList<Sequence> masking = new ArrayList<Sequence>();
		for(int i = 0; i < sequences.size(); i++){
			Sequence seq = sequences.get(i);
	 ...
	 * @param seqs
	 */
	public void maskSequences(ArrayList<Sequence> seqs) {

		...
		//try {
			...
			for(String scaffold: scaffoldsToSeqNamesEtc.keySet()) {
				...
				for(String readName: readNamesToPiecesHash.keySet()) {
					...
					// Note that if we get here, and false is the setting for allowGaps, then there is only one piece and the split method will work just fine.
					//if(allowGaps.compareTo("split") == 0 || allowGaps.compareTo("false") == 0) {
						int pieceIndex = 0;
						for(String piece: pieceToStartPosHash.keySet()) {
							...
							ArrayList<String> piecesList = new ArrayList<String>(); piecesList.add(piece);
							boolean seqHasGaps = newSeq.addPieces(piecesList, startPos);
					...
					//}
					/*else if(allowGaps.compareTo("true") == 0) {
						Sequence newSeq = new Sequence();
						...
						throw new Exception("Look, you can't just give me options I don't understand ("+ allowGaps + "). What have I done to you?");
					}*/
				}
			}
		//}

...
public class Haplotype {
	private ArrayList<Sequence> haplotype;
	...
	public Haplotype(MultipleAlignment theAlignment) {
		haplotype = new ArrayList<Sequence>();
	...
	public void addSequence(Sequence theSequence) {
		if(!haplotype.contains(theSequence)) {
			...
			ArrayList<SNP> possibleNewSnps = theSequence.getSNPsCovered();
	...
	public String getConsensusAtAllSNPs() {
		StringBuffer sb = new StringBuffer();
		ArrayList<SNP> allSnps = alignment.getSNPs();
		if(allSnps.size() == 0) {
			sb.append("!");
		}
		for(int k = 0; k < allSnps.size(); k++) {
			SNP theSNP = allSnps.get(k);
...
/**
	...
	public int numSNPsCovered() {
		HashSet<SNP> uniqSNPs = new HashSet<SNP>();
		//for(Sequence seq: haplotype) {
		/*for(int i = 0; i < haplotype.size(); i++) {
			Sequence seq = haplotype.get(i);
			ArrayList<SNP> snpsCovered = seq.getSNPsCovered();
			//for(SNP snp: snpsCovered) {
			for(int j = 0; j < snpsCovered.size(); j++) {
				SNP snp = snpsCovered.get(j);
				if(!uniqSNPs.contains(snp)) {
					uniqSNPs.add(snp);
				}
			}
		}*/

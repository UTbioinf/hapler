...
public class Reconstructor {
	...
	public ReconstructedConsensus computeReconstructedConsensus(MultipleAlignment alignment) {
		ArrayList<SNP> theSnpList = alignment.getSNPs();
	...
	public ReconstructedConsensus evaluateConsensus(MultipleAlignment alignment, Sequence consensus, boolean punishGapsInConsensus) throws Exception {
		...
		if(theSnpList.size() == 0) {
			...
			ReconstructedConsensus recon = new ReconstructedConsensus();
			ArrayList<SNP> coveredSnps = new ArrayList<SNP>();
	 ...
	 * @param snpList
	 */
	private HashMap<SNP, ArrayList<SnpDAGNode>> setupNodesHapsComptabileWithSeq(ArrayList<HaplotypeBlock> haploBlocks, ArrayList<SNP> theSnpList, Sequence theSeq) throws Exception {
		...
		ArrayList<SNP> snpList = theSnpList;
	 ...
	 * @param snpList
	 */
	private HashMap<SNP, ArrayList<SnpDAGNode>> setupNodesAllHaps(ArrayList<HaplotypeBlock> haploBlocks, ArrayList<SNP> theSnpList) {
		// get snps and haplotypes
		ArrayList<Haplotype> haplotypes = new ArrayList<Haplotype>();
		HashMap<SNP, ArrayList<SnpDAGNode>> snpToSnpDAGNodes = new HashMap<SNP, ArrayList<SnpDAGNode>>();
	 ...
	 */
	private void computeBackPointers(HashMap<SNP, ArrayList<SnpDAGNode>> snpToSnpDAGNodes, ArrayList<SNP> snpList) {
		Collections.sort(snpList, new SNPPosComparator());
		// base of the dynamic program
		ArrayList<SnpDAGNode> firstNodes = snpToSnpDAGNodes.get(snpList.get(0));
		...
		for(int i = 0; i < snpList.size()-1; i++) {
			SNP thisSnp = snpList.get(i);
			SNP nextSnp = snpList.get(i+1);
	 ...
	 * @return
	 */
	private ArrayList<SnpDAGNode> minSNPCompatibilityList(HashMap<SNP, ArrayList<SnpDAGNode>> snpToSnpDAGNodes, ArrayList<SNP> snpList) {
